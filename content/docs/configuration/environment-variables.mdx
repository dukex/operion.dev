...
Command: `./bin/operion-worker run`
...
Command: `./bin/operion-dispatcher run`
...
# MySQL (if supported)
DATABASE_URL=mysql://user:password@localhost:3306/operion?parseTime=true
```
...
./bin/operion-api run --port 8080 --database-url "file://./data.json" --event-bus "kafka" --log-level "debug"
```
...
# Using command-line flags
./bin/operion-worker run --worker-id "worker-01" --database-url "file://./data.json" --event-bus "kafka"
```
...
# Using command-line flags
./bin/operion-dispatcher run --webhook-port 8085 --database-url "file://./data.json" --event-bus "kafka"
```
